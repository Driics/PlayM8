...
import ru.driics.playm8.utils.cache.BitmapsCache;
import ru.driics.playm8.utils.cache.DispatchQueue;
import ru.driics.playm8.utils.cache.DispatchQueuePool;
import ru.driics.playm8.utils.cache.DispatchQueuePoolBackground;
...
public class RLottieDrawable extends BitmapDrawable implements Animatable, BitmapsCache.Cacheable {
    ...
    protected void recycleResources() {
        ...
        AndroidUtils.INSTANCE.recycleBitmaps(bitmapToRecycle);
    ...
    protected boolean scheduleNextGetFrame() {
        ...
        if (shouldLimitFps && Thread.currentThread() == MyApplication.applicationHandler.getLooper().getThread()) {
        ...
        if (!isInvalid && renderingBitmap != null) {
            ...
            if (!drawInBackground) {
                ...
                if (applyTransformation) {
                    ...
                    this.needScale = !(Math.abs(rect.width() - width) < ViewUtils.INSTANCE.getToPx(1) && Math.abs(rect.height() - height) < ViewUtils.INSTANCE.getToPx(1));
                }
                scaleX = this.scaleX;
                scaleY = this.scaleY;
                ...
                needScale = !(Math.abs(rect.width() - width) < ViewUtils.INSTANCE.getToPx(1) && Math.abs(rect.height() - height) < ViewUtils.INSTANCE.getToPx(1));
            }
            if (!needScale) {
    ...
    public void updateCurrentFrame(long time, boolean updateInBackground) {
        ...
        } else if (refreshRating <= 60 || (updateInBackground && refreshRating <= 80)) {
    ...
    public void drawFrame(Canvas canvas, int frame) {
        ...
        canvas.drawBitmap(backgroundBitmap, tmp, getBounds(), getPaint());
    }
    ...
    public void checkCache(Runnable onReady) {
        if (bitmapsCache == null) {
            AndroidUtils.INSTANCE.runOnUIThread(onReady);
            return;
        }
    ...
    private static class LottieMetadata {
        float fr;
        float op;
        float ip;
    }
