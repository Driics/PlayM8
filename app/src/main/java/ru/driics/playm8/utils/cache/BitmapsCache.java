...
import ru.driics.playm8.components.lottie.RLottieDrawable;
...
public class BitmapsCache {
    ...
    private final static int N = Utils.INSTANCE.clamp(Runtime.getRuntime().availableProcessors() - 2, 6, 1);
    ...
    private Runnable cleanupSharedBuffers = new Runnable() {
        ...
        public void run() {
            ...
            if (!sharedBuffers.isEmpty()) {
                AndroidUtils.INSTANCE.runOnUIThread(cleanupSharedBuffers, 5000);
            } else {
    ...
    public BitmapsCache(File sourceFile, Cacheable source, CacheOptions options, int w, int h, boolean noLimit) {
        ...
        useSharedBuffers = w < ViewUtils.INSTANCE.getToPx(60) && h < ViewUtils.INSTANCE.getToPx(60);
        ...
        try {
            ...
            while (true) {
                ...
                bitmapCompressExecutor.execute(() -> {
                    ...
                    int size = byteArrayOutputStream[finalIndex].getCount();
...
//        cancelled.set(true);
    ...
    public int getFrame(int index, Bitmap bitmap) {
        ...
        try {
            ...
            index = Utils.INSTANCE.clamp(index, frameOffsets.size() - 1, 0);
